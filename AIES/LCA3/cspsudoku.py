...
def readCSPFromFile(pathToFile):
    ...
    with open(pathToFile) as file:
        matrix = [[int(x) if x.isdigit() else 0 for x in line.strip() if x.isdigit() or x == "X"] for line in file if
                  "-" not in line]
    ...
    domains = {"ABCDEFGHI"[y] + "123456789"[x]: {1, 2, 3, 4, 5, 6, 7, 8, 9} if matrix[y][x] == 0 else {matrix[y][x]} for
               y in range(0, 9) for x in range(0, 9)}
